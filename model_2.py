...
interpreter = tfl.Interpreter(model_path="model_2.tflite")
...
for index, (x,y) in enumerate(zip(X_test, y_test)):
	...
	print(str(index) + ") Predicted CO2: ", y_pred_i[0][0])

	# If you have a reference output for comparison
	print(str(index) + ") Actual CO2: ", y[0][0])
...
plt.plot(y_test_plot.reshape(-1), color='blue', label='Actual CO2')
plt.plot(y_pred_plot.reshape(-1), color='red', label='Predicted CO2', linestyle='--')
plt.xlabel('N. Points')
plt.ylabel('CO2')
